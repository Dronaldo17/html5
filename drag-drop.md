...
所有 drag 和 drop 事件的时间监听器都接收一个 __Event__ 对象作为参数，它有一个叫做 __dataTransfer__ 的只读属性。__event.dataTransfer__ 返回与事件相关的 __DataTransfer__ 对象，如下所示：
...
- 给 __dragstart__ 设置一个时间监听器存储拖拽数据。
